...
def sample_and_group(xyz, points, npoint, nsample, radius, use_xyz=True):
    """
    先进行fps采样，保留npoint个点，然后在这些点的球形区域内取nsample个点，得到npoint个分组，每个分组有nsample个成员
    每个成员是一个三维点。然后将该组所有点的坐标归一化到中心点的坐标系下。
    对特征向量的处理类似，得到(B, d, npoint, nsample)的聚合特征，然后拼接坐标后作为新的特征向量
    Args:
        xyz: Tensor, (B, 3, N)
        points: Tensor, (B, f, N)
        npoint: int
...
def sample_and_group_knn(xyz, points, npoint, k, use_xyz=True, idx=None):
    ...
    """
    xyz_flipped = xyz.permute(0, 2, 1).contiguous() # (B, N, 3)
    new_xyz = gather_operation(xyz.contiguous(), furthest_point_sample(xyz_flipped, npoint)) # (B, 3, npoint)
    if idx is None:
        idx = query_knn(k, xyz_flipped, new_xyz.permute(0, 2, 1).contiguous())
    grouped_xyz = grouping_operation(xyz.contiguous(), idx) # (B, 3, npoint, nsample)
    grouped_xyz -= new_xyz.unsqueeze(3).repeat(1, 1, 1, k)

    if points is not None:
        grouped_points = grouping_operation(points.contiguous(), idx) # (B, f, npoint, nsample)
        if use_xyz:
            new_points = torch.cat([grouped_xyz, grouped_points], 1)
        else:
            new_points = grouped_points
