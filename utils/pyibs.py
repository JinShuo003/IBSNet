...
class IBS():
    def __init__(self, points0, points1, n=500, use_angle=True, use_dist=True, dist_power=2, clip='sphere',
                 bounded=True) -> None:
        # input
    ...
    def sample_points(self):
        ...
        cos = np.abs(np.sum((vec0 * norms) / (
                    (np.linalg.norm(vec0, axis=-1) * np.linalg.norm(norms, axis=-1))[:, None] + np.finfo(float).eps),
                            axis=-1))
        alpha = np.arccos(cos)
        self.alpha = alpha
