...
def get_network(specs):
    ...
    encoder_obj1 = PN2_Transformer_Encoder()
    encoder_obj2 = PN2_Transformer_Encoder()
    # decoder = Decoder(latent_size, **specs["NetworkSpecs"])
    decoder = CombinedDecoder(latent_size, **specs["NetworkSpecs"])
    IBS_Net = IBSNet_transformer(encoder_obj1, encoder_obj2, decoder, num_samp_per_scene)
...
def train(IBS_Net, sdf_train_loader, lr_schedules, optimizer, epoch, specs, tensorboard_writer, TIMESTAMP):
    ...
    loss_udf = torch.nn.L1Loss(reduction="sum")
    ...
    for pcd1, pcd2, sdf_data, indices in sdf_train_loader:
        ...
        udf_gt = sdf_data[:, 3].unsqueeze(1)

        udf_pred = IBS_Net(pcd1, pcd2, xyz)

        # 计算每个点的平均l1-loss
        batch_loss = loss_udf(udf_pred, udf_gt.to(device)) / (scene_per_batch * num_samp_per_scene)
