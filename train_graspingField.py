...
import utils
import networks.models as arch
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for i, (hand_samples, hand_labels, obj_samples, obj_labels,
            ...
            for _subbatch in range(batch_split):
                ...
                if '1decoder' in model_type:
                    ...
                    if hand_branch and obj_branch:
                        ...
                        if ignore_other or epoch < start_additional_loss:
                            mask_hand = torch.cat(
                                [torch.ones(hand_samples.size()[:2]), torch.zeros(obj_samples.size()[:2])], 1)
                            mask_hand = (mask_hand.cuda()).reshape(num_samp_per_scene * scene_per_subbatch).unsqueeze(1)
                            mask_obj = torch.cat(
                                [torch.zeros(hand_samples.size()[:2]), torch.ones(obj_samples.size()[:2])], 1)
                ...
                if model_type == 'PC_2encoder1decoder_VAE':
                    pred_sdf_hand, pred_sdf_obj, pred_class, kl_loss, z_hand = encoderDecoder(encoder_input_hand,
                                                                                              encoder_input_obj,
                                                                                              xyz_hand)
                elif model_type == 'pc+1encoder1decoder':
                    pred_sdf_hand, pred_sdf_obj, pred_class = encoderDecoder(encoder_input_hand, encoder_input_obj,
                                                                             xyz_hand)
                ...
                if do_penetration_loss:
                    pen_loss = torch.max(-(scaled_pred_sdf_hand + scaled_pred_sdf_obj),
                                         torch.Tensor([0]).cuda()).mean() * penetration_loss_weight
                    if epoch >= start_additional_loss:
                        loss = loss + pen_loss

                if do_contact_loss:
                    alpha = 1. / contact_loss_sigma ** 2
                    contact_loss = torch.min(alpha * (scaled_pred_sdf_hand ** 2 + scaled_pred_sdf_obj ** 2),
                                             torch.Tensor([1]).cuda()).mean() * contact_loss_weight
                    if epoch >= start_additional_loss:
            ...
            if ((epoch - 1) * len(sdf_loader) + i) % log_frequency_step == 0:
                ...
                print(
                    'step {}, loss {:.5f}, hand loss {:.5f}, object loss {:.5f}:, classifier loss {:.5f}, penetration loss {:.5f}, contact loss {:.5f} '.format(
                ...
                if 'VAE' in model_type:
                    ...
                    writer.add_scalar('KL_loss_raw_1e-3', kl_loss_raw.item() * 1000.0,
                                      (epoch - 1) * len(sdf_loader) + i)
